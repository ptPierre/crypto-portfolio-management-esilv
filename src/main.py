...
from portfolio.allocator import TargetAllocator
from portfolio.rebalancer import compute_rebalance_orders
from strategies.momentum import momentum_scores, select_top_k
...
class PortfolioBot:
    def __init__(self):
        ...
        self.week_every = int(os.getenv("WEEK_EVERY_N_BARS", "24"))
        self.month_every = int(os.getenv("MONTH_EVERY_N_BARS", "96"))
        ...
        self.use_momentum = os.getenv("USE_MOMENTUM", "true").lower() == "true"
        self.lookback = int(os.getenv("MOMENTUM_LOOKBACK_BARS", "72"))
        self.top_k = int(os.getenv("MOMENTUM_TOP_K", "10"))
        ...
        self.allocator = TargetAllocator("config/target_allocation.json")
        self.target_weights = self.allocator.weights()

        # Universe
        self.universe_requested = [s for s in self.allocator.symbols() if s != "USDC"]
        ...
        self.last_momentum_top: Optional[List[str]] = None
        ...
        if self.mode == "live":
            ...
            if self.N < max(10, self.lookback + 5):
    ...
    def _init_live(self) -> None:
        ...
        bootstrap = max(self.live_bootstrap_bars, self.lookback + 10)
    ...
    def _eligible_symbols(self, i: int) -> List[str]:
        eligible = list(self.universe)
        if not self.use_momentum:
            return eligible

        scores = momentum_scores(self.price_history, i, self.lookback)
        if not scores:
            return eligible

        k = min(self.top_k, len(scores))
        top = select_top_k(scores, k)
        self.last_momentum_top = top
        return [s for s in eligible if s in set(top)]

    def _normalized_weights(self, eligible: List[str]) -> Dict[str, float]:
        w = {s: float(self.target_weights.get(s, 0.0)) for s in eligible}
        ssum = sum(w.values())
        if ssum <= 0:
            n = len(eligible)
            if n == 0:
                return {}
            return {s: 1.0 / n for s in eligible}
    ...
    def _append_live_bar_to_history(self, bar_ts: int, rows: Dict[str, Tuple[int, str, float]]) -> None:
        ...
        """
        keep = max(self.live_bootstrap_bars, self.lookback + 50)
        ...
        for i in range(self.N):
            ...
            self.status_writer.write(
                BotStatus(
                    ...
                    momentum_top_k=self.top_k,
                    momentum_last_top=self.last_momentum_top,
                )
            )

            do_weekly = (i % max(1, self.week_every) == 0) and (i >= self.lookback)

            if do_weekly:
                invest_usd = float(self.dca_rate) * float(equity)
                invest_usd = min(invest_usd, max(0.0, self.portfolio.cash - reserve_min))

                if invest_usd > 0:
                    eligible = self._eligible_symbols(i)
                    w = self._normalized_weights(eligible)
                    ...
                    for sym, wi in w.items():
                        ...
                        self.tracker.log_trade(
                            ...
                            f"DCA weekly invest {usd:.2f} (momentum={self.use_momentum})"
                            if res.get("success")
                            else (res.get("error", "")),
                        )
            ...
            if self.rebalance_period == "monthly":
                do_rebalance = (i % max(1, self.month_every) == 0) and (i >= self.lookback)
            elif self.rebalance_period == "quarterly":
                do_rebalance = (i % max(1, 3 * self.month_every) == 0) and (i >= self.lookback)

            if do_rebalance:
                eligible = self._eligible_symbols(i)
                tw = self._normalized_weights(eligible)

                current_vals = self._current_values(prices)
                orders = compute_rebalance_orders(prices, current_vals, tw, equity, min_trade_usd=20.0)

                sells = [(s, d) for s, d in orders if d < 0]
                buys = [(s, d) for s, d in orders if d > 0]
                ...
                for sym, delta in sells:
                    ...
                    self.tracker.log_trade(
                        ...
                        res.get("price", px),
                        bool(res.get("success", False)),
                        "Rebalance sell" if res.get("success") else res.get("error", ""),
                    )
                ...
                for sym, delta in buys:
                    ...
                    self.tracker.log_trade(
                        ...
                        res.get("price", px),
                        bool(res.get("success", False)),
                        "Rebalance buy" if res.get("success") else res.get("error", ""),
                    )
    ...
    def _run_live(self) -> None:
        ...
        while True:
            ...
            # Append to in-memory history so momentum & index-based code still works
            self._append_live_bar_to_history(bar_ts, rows)

            # i is the "bar index" for our rolling DataFrames
            # Since we keep rolling windows, use last index in any df
            # (they should be aligned in time roughly; we pick their last row index)
            i = min(len(df) for df in self.price_history.values()) - 1
            if i < 0:
                time.sleep(min_sleep)
                continue

            # Log latest closes to CSV for dashboard
            for sym, (ts_s, dt_str, close) in rows.items():
                self.price_logger.log_close(
                    timestamp=int(ts_s),
                    dt_str=str(dt_str),
            ...
            # DCA / rebalance triggers based on "bar index"
            do_weekly = (i % max(1, self.week_every) == 0) and (i >= self.lookback)
                ...
                if invest_usd > 0:
                    ...
                    for sym, wi in w.items():
                        ...
                        self.tracker.log_trade(
                            ...
                            res.get("price", px),
                            bool(res.get("success", False)),
                            f"DCA weekly invest {usd:.2f} (live, momentum={self.use_momentum})"
                            if res.get("success")
                            else (res.get("error", "")),
                        )
            ...
            if do_rebalance:
                ...
                for sym, delta in sells:
                    ...
                    self.tracker.log_trade(
                        ...
                        res.get("price", px),
                        bool(res.get("success", False)),
                        "Rebalance sell (live)" if res.get("success") else res.get("error", ""),
                    )
                ...
                for sym, delta in buys:
                    ...
                    self.tracker.log_trade(
                        ...
                        res.get("price", px),
                        bool(res.get("success", False)),
                        "Rebalance buy (live)" if res.get("success") else res.get("error", ""),
                    )

            # sleep a bit; we also gate on "new bar", so this is just polling pace
            time.sleep(min_sleep)
